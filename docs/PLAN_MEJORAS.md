...
| --------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **H1 · Consolidar**         | **6 de 8 cerrados** | ✅ #1 consolidación CSS (fase 1, exacta) · ✅ #2 componentes (`PanelHead`, inspector partido; falta `Row/Tile/Field`) · ✅ #3 E2E estables + auditoría en CI · ✅ #4 regresión visual (24 estados) · ⏳ #5 observabilidad · ⏳ #6 bus multi-réplica · ⏳ #7 roles · ✅ #8 páginas de error/404 (esqueletos pendientes) |
...
| **Componentes monolíticos**                                                                                            | `InspectorPanel.tsx` 956 líneas, `TopBar.tsx` 542, `Library.tsx` 532, `useCommands.ts` 297.       | Pruebas unitarias de UI inexistentes; cambios con efectos laterales.                        |
| **Sin componentes de sistema**                                                                                         | No existen `Panel`, `Row`, `Tile`, `Field`; cada superficie repite el markup.                     | La homologación puede desviarse en la próxima pantalla.                                     |
...
| **Servidor de un solo proceso**                                                                                        | Presencia/eventos en memoria; sin bus (LISTEN/NOTIFY o Redis); un workspace; sin roles.           | No escala a réplicas ni a equipos con permisos.                                             |
| **Lienzo de familia única**                                                                                            | Solo frontera › grupo › contenedor › servicio; sin nodos libres, notas, imágenes ni texto suelto. | Cubre arquitectura cloud, no diagramas generales (F2/F6 del plan maestro).                  |
| **Sin observabilidad**                                                                                                 | No hay trazas, métricas ni logs estructurados en las rutas API.                                   | Incidentes en producción sin diagnóstico.                                                   |
...
| 2   | **Extraer componentes de sistema en React** ✅ fase 1 (`PanelHead`, inspector partido) | Deuda          | M        | `src/components/ui/{Panel,PanelHead,Row,Tile,Field,ChipGroup,Segmented,NumberField,Kbd}.tsx` con pruebas de render (`@testing-library/react` + happy-dom). Migrar explorador, paleta, selector, historial, análisis y menús a ellos. El inspector se divide en `ShapeInspector`, `ConnectorInspector`, `MetaSection`, `AppearanceSection`, `PositionSection`.                                                                    |
...
| 5   | **Observabilidad mínima**                                                              | Operación      | M        | Logs JSON con `requestId`, `userId`, `diagramId` en `src/server/http.ts`; métricas Prometheus en `/api/metrics` (latencia por ruta, 4xx/5xx, conflictos 412, guardados, sesiones); trazas OpenTelemetry opcionales por `OTEL_EXPORTER_OTLP_ENDPOINT`.                                                                                                                                                                            |
| 6   | **Presencia multi-réplica**                                                            | Plataforma     | M        | Bus `LISTEN/NOTIFY` de PostgreSQL (ya hay `pg`) para difundir `saved`/`presence`/`deleted` entre procesos; fallback en memoria cuando no hay DB. Prueba con dos procesos en `compose.server.yaml`.                                                                                                                                                                                                                               |
| 7   | **Roles por diagrama**                                                                 | Plataforma     | L        | Tabla `diagram_members(diagram_id, user_id, role: owner                                                                                                                                                                                                                                                                                                                                                                          | editor | viewer)`; el propietario invita por correo (usuarios ya vistos por OIDC); rutas y SSE respetan el rol; UI: menú "Compartir" con lista de personas y selector de rol; lector en modo solo lectura con presencia. |
...
| 10  | **Presentación**                                                | Visual/UX         | M          | Modo presentación (`F5`/botón): oculta el chrome, fondo del tema, navega por vistas con flechas, muestra título y leyenda de tonos; exporta a PDF multipágina (una página por vista).                                                                                                                                                         |
...
| 12  | **Notas, texto y regiones**                                     | Editor            | L          | Nuevos tipos de forma: `note` (post-it con markdown ligero), `text` (etiqueta libre), `region` (zona coloreada sin semántica). Entran en esquema, reducer, render, DSL (`notes:`), exportaciones y análisis (ignorados). Es el primer paso de F2 sin romper la familia cloud.                                                                 |
...
| 14  | **Biblioteca de iconos en servidor**                            | Plataforma        | M          | Tabla `icons` por workspace; el cliente sincroniza `aion-studio-custom-icons` con `/api/icons`; deduplicación por hash del SVG; cuota por workspace.                                                                                                                                                                                          |
...
| 20  | **Plantillas propias**                                          | Biblioteca        | S          | "Guardar como punto de partida" desde el editor; las plantillas del usuario aparecen en la portada con miniatura real; se pueden compartir en el workspace (H1 #7).                                                                                                                                                                           |
