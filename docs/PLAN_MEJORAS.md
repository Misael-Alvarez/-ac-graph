...
| 17  | **Exportación draw.io y PPTX** ✅ draw.io · PPTX pendiente         | Interoperabilidad | M / L      | draw.io hecho (2026-09-14): `toDrawio` escribe mxGraph XML sin comprimir, una página por vista, iconos incrustados como `data:image/svg+xml` en base64, rutas manuales como puntos, puertos como `exitX/entryX`, etiqueta en su posición (`labelAt`), tema claro/oscuro y metadatos opcionales; entrada en el menú Exportar, la paleta y la auditoría; abierto en el visor de diagrams.net (7/7 iconos). PPTX (`pptxgenjs` o ZIP propio, una diapositiva por vista) sigue pendiente.                                                                                                                                                                    |
