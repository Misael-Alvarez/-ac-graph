...
- **Estado del árbol:** limpio en `d133470` y **empujado** (`origin/main` = `d133470`, 2026-09-15; CI verde: `verify` + `docker-smoke`; «Deploy to AWS» omitido hasta que existan sus variables en GitHub). Commits de esta etapa, por tema:
  ...
  - `0ed5c61` — Hold a shape still and let the hand do more: a lock nothing moves, a lasso that adds or takes away, an Alt-drag that leaves a copy, and guides while resizing
...
- Docker Desktop se arranca con `open -g -a Docker` (el daemon tarda ~30 s). El 2026-09-15 se ejecutaron con él las pruebas `.pg.test.ts` (1578 en total) y `audit:roles` (47/47, login con contraseña); el contenedor de 3080 sigue con la imagen de `0bb02b1`.
...
# Auditoría del login, roles, iconos compartidos y comentarios en vivo (modo servidor con contraseñas, tres navegadores; siembra y limpia sus propios usuarios/sesiones; 47 comprobaciones)
# 1) app en modo servidor contra el PostgreSQL desechable (sin OIDC: login con contraseña; el esquema lo migra la propia app al primer acceso con cookie):
(PORT=3101 HOSTNAME=127.0.0.1 ANTHROPIC_API_KEY= DATABASE_URL=postgres://postgres:test@127.0.0.1:55433/acgraph_test APP_URL=http://127.0.0.1:3101 nohup npm start > /tmp/acgraph-3101.log 2>&1 &)
...
Desde el 2026-09-15, en http://127.0.0.1:3080 corre el proyecto **`acgraph-login`** en **modo servidor con login por contraseña** (imagen construida en `0ed1c44`; `/api/config` → `provider: local, signup: true`): `docker compose -f compose.yaml -f compose.server.yaml -p acgraph-login` con `POSTGRES_PASSWORD=acgraph-dev-2026` (solo desarrollo, loopback) y `APP_URL=http://127.0.0.1:3080`; su PostgreSQL está publicado en `127.0.0.1:55432` con volumen propio `acgraph-login_postgres-data`. Para verlo: abrir 3080 → «¿Aún no tienes cuenta? Créala». Cuentas desde terminal: `DATABASE_URL=postgres://acgraph:acgraph-dev-2026@127.0.0.1:55432/acgraph npm run users -- list`. **El login no existe en modo local** (sin `DATABASE_URL`): quien abra `next dev` o la imagen sin variables verá la portada directamente, por diseño.

El proyecto anterior `acgraph-foundation` (modo local, imagen de `0bb02b1`) está **parado**, no borrado: su volumen `acgraph-foundation_postgres-data` (contraseña desconocida; usuarios semilla Ana Torres, Luis Pérez de auditorías) sigue ahí. Para volver a él: `docker compose -p acgraph-login down` y `docker compose -p acgraph-foundation start app`. El pie de la portada y el menú de cuenta muestran el **sello de compilación** (`NEXT_PUBLIC_BUILD_STAMP`): si la hora no coincide con el último build, el navegador sirve caché (`Cmd+Shift+R`).
...
- **Bloqueo, lazo con modificadores y Alt+arrastre (§3.1 del plan, 2026-09-15):** `Shape.locked` (esquema 6) se filtra **en el reducer** (`movable`/`GEOMETRY_KEYS`; `isLocked` lee ancestros), nunca control por control; `setLocked { ids, locked }` es la acción para la selección (un paso de deshacer; escribe la bandera solo si está puesta) y `setShapeProps` con `locked: false` deja pasar geometría en el mismo patch. Una copia (`pasteShapes`, fantasma de `previewDuplicate`) **nunca** hereda el bloqueo. El candado es chrome (`.lock-mark` en la capa de overlay de `Canvas.tsx`, en píxeles de pantalla; no se exporta ni se presenta); el asa de redimensionar no se dibuja en una forma bloqueada y `startDrag` no arranca sobre ella (nada que se mueva y vuelva). El modificador se lee **al empezar** el gesto (`startLasso`: Alt→`subtract`, Shift→`add`; `startDrag`: Alt→clon) y una pulsación en la hoja con Shift/Alt no vacía la selección. Los fantasmas llevan ids `ghost-…` (`GHOST_PREFIX`): ningún selector `[data-shape-id^="grp_"]` ni contador los ve; las guías del fantasma se calculan con su id para que el original sea un vecino. Tras soltar un Alt+arrastre la copia queda seleccionada (`selectNextCreated` + `E.outermost(view, doc.lastCreated)`). `computeResizeGuides` comparte `alignmentCandidates` con `computeAlignGuides`; un imán bajo el tamaño mínimo se descarta con su guía. En la hoja de atajos, los gestos de puntero son pseudoacordes (`Shift+Lasso`, `Alt+Drag`…) que `chordOf` nunca produce: inertes y únicos. El minimapa solo mueve la cámara si **él** capturó el puntero (`hasPointerCapture`): un arrastre del lienzo que pase por encima no lo toca.
...
- **Login (2026-09-15): contraseña propia por defecto; Authentik opcional.** El usuario pidió «un login normal, sin Authentik por ahora». Modo servidor = `DATABASE_URL` + `APP_URL`; proveedor `local` salvo que `OIDC_ISSUER` **y** `OIDC_CLIENT_ID` estén ambos (uno solo apaga el modo servidor: nunca caer en silencio a contraseñas). Todo en `src/server/auth/password.ts`: scrypt de Node (N=2¹⁵, r=8, p=3, `maxmem` 64 MiB; parámetros dentro de la cadena, subibles), cuenta local = `users` con `issuer='local'` y `subject=lower(email)` (la unicidad de correo la da la constraint existente; **no** añadir índice por email), `password_hash` nullable (migración 10). Un correo inexistente y una contraseña mala son el mismo 401 y cuestan lo mismo (`decoy`). Límite de intentos con el `RateLimiter` de la IA (singletons `loginLimiter`/`signupLimiter`; `resetServerSingletons` los limpia). Las rutas de auth declaran su cara (`withServerMode(..., { provider })`) y la guarda responde 404 **antes** de `ensureSchema()`; la validación del cuerpo va después (tests de eso en la suite PG, no en `serverMode.test.ts`). `AUTH_SIGNUP=closed` deja las altas a `npm run users` (`scripts/users.mjs`, usa `createLocalUser`/`setPassword` con su propio `pg.Pool`). Nunca registrar el correo en el log de un rechazo. El provider de Authentik `ac-graph` sigue sin existir; cuando toque: `docs/AUTHENTIK.md` §1–§6.
- **AWS (2026-09-15):** la base está en `deploy/aws/terraform` (ECS Fargate + ALB/ACM + RDS 17 + Secrets + ECR + rol OIDC para GitHub) y `.github/workflows/deploy-aws.yml`; **no se ha aplicado** desde esta máquina (sin credenciales, a propósito). Decisiones: sin NAT (tareas en subredes públicas con SG solo desde el ALB), `verify-full` hacia RDS con el bundle de Amazon **dentro de la imagen** (`deploy/aws/rds-global-bundle.pem`; `.dockerignore` **y** `.gitignore` lo readmiten con `!deploy/aws/rds-global-bundle.pem` después de sus reglas `*.pem` — el primer push lo dejó fuera y el `docker-smoke` de CI falló por ello), Terraform es dueño de la forma de la task definition y CI de la etiqueta (`ignore_changes = [task_definition]`), `idle_timeout` 3600 por los SSE. Guías: `docs/AWS.md` (inglés, runbook) y `docs/GUIA_RAPIDA_AWS.md` (español, para quien lo monta).
...
- Edición del lienzo (§3.1): gestos en `src/components/editor/hooks/usePointerTools.ts` (`Interaction.drag.clone`, `lasso.mode`, `resize.guides`), `previewDuplicate`/`GHOST_PREFIX` en `lib/editor/preview.ts`, `computeResizeGuides`/`alignmentCandidates` en `engine/layout.ts`, `isLocked` en `engine/model.ts`, `setLocked` en `actions.ts`/`reducer.ts`, botón en `ShapeInspector.tsx` (`data-lock`), fila `lock` en `ContextMenu.tsx`, comando `lock` en `useCommands.ts`, `Mod+L` y grupo `shortcuts.pointer` en `shortcuts.ts`, `.lock-mark` en `Canvas.tsx`/`globals.css`, `locked` en el DSL (`schema`/`serialize`/`parse`), E2E `e2e/selection.spec.ts`.
...
- Servidor: `src/server/**`, rutas `src/app/api/**`; CLI y MCP en `bin/`. Login: `src/server/env.ts` (`readServerEnv`, `oidcEnv`, `authProvider`, `signupOpen`), `src/server/auth/{password,session,oidc}.ts`, guarda `src/server/handler.ts` (`AuthGuardOptions.provider`), rutas `src/app/api/auth/{login,register,logout,callback,me}`, cliente `AuthProvider.tsx` (`signIn`/`register`) + `SignInGate.tsx` (`PasswordForm`), `AppConfigSchema.auth.{provider,signup}`, CLI `scripts/users.mjs`; pruebas `password.test.ts`, `password.pg.test.ts`, `testEnv(schema, 'local')`.
- Despliegue: `Dockerfile` (+ bundle CA de RDS), `compose*.yaml`, `deploy/aws/terraform/*.tf`, `.github/workflows/deploy-aws.yml`, `docs/AWS.md`, `docs/GUIA_RAPIDA_AWS.md`. Colaboración: `src/server/collab/{events,presence,stream,bus,collaboration}.ts` (hub local, roster, SSE, transporte `LISTEN/NOTIFY`, coordinador por réplica). Roles: `src/server/diagrams/{repository,errors,schemas}.ts`, rutas `src/app/api/diagrams/[id]/members/**`, cliente `src/lib/store/httpRepository.ts` (`MembersApi`), solo lectura `src/lib/editor/readOnly.ts` + `readOnly` en `EditorProvider`, UI `ShareDialog.tsx` (`SharePeople`) y `Library.tsx`.
...
- Pruebas: `src/**/*.test.ts` (1578 con `TEST_DATABASE_URL`; 1490 sin PostgreSQL), `e2e/*.spec.ts` (175 funcionales + `visual.spec.ts` 24), líneas base en `e2e/__screenshots__/`; `audit:controls` 134, `audit:roles` 47.

## 8. Siguiente paso recomendado

El proyecto queda **cerrado para desplegar**: quien lo monte sigue `docs/GUIA_RAPIDA_AWS.md` (Terraform → primera imagen → cuenta → variables de GitHub → cerrar altas). Lo primero que conviene hacer después de la primera aplicación real:

1. Confirmar `terraform apply` en la cuenta (no se ha ejecutado desde esta máquina) y anotar en `AWS.md` cualquier ajuste que pidiera la cuenta (límites de servicio, proveedor OIDC de GitHub ya existente → `terraform import`).
2. Cerrar las altas (`auth_signup = "closed"`) cuando el equipo esté dentro; decidir si hace falta cambio de contraseña en la interfaz o recuperación por correo (hoy: `npm run users -- passwd`).
3. Del plan (`PLAN_MEJORAS.md`): H2 #18/#19 (IA que edita; revisión asistida) necesitan `ANTHROPIC_API_KEY` real; H3 (CRDT, PWA, multi-workspace, escaneo cloud). Backlog menor de §3.
