...
1. **Portada entregada en dos pasadas** (ver `CHECKPOINTS.md` «Portada: arreglos, rediseño y referencia del usuario»). Si el usuario quiere seguir con el front por el mismo criterio: diálogo «Nuevo diagrama» con plantillas propias y la anatomía de tarjeta de la portada, título de los duplicados en el idioma de la interfaz, y una pestaña «Catálogo» solo si se decide crear una página de catálogo de servicios.
