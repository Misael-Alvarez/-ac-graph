...
Última actualización: 2026-09-10 (H1 cerrado; H2 #10 modo presentación entregado; ver commits en §1).
...
- **Estado del árbol:** limpio tras el commit de H2 #10 (ver `git log -1`). Commits de esta etapa, por tema:
  ...
  - (siguiente) — Present the diagram: no chrome, one view per arrow key, a legend, and a PDF page per view
...
# E2E funcionales (139), visuales (24) y auditoría de controles (88)
...
`acgraph-foundation-app-1` en **modo local** en http://127.0.0.1:3080 con la imagen reconstruida el 2026-09-10 tras el commit de H2 #10 (presentación; `/api/config` → `{"mode":"local"}`; `docker compose -p acgraph-foundation logs --no-log-prefix app` muestra JSON; `/api/metrics` responde; en modo local `acgraph_collab_bus_connected` es 0 porque el bus es en memoria). El volumen `postgres-data` del proyecto tiene contraseña desconocida en esta sesión (no hay `.env.local`); para pruebas se usa el contenedor desechable de la sección 3. El pie de la portada y el menú de cuenta muestran el **sello de compilación** (`NEXT_PUBLIC_BUILD_STAMP`): si la hora no coincide con el último build, el navegador sirve caché (`Cmd+Shift+R`). Volumen `postgres-data` conserva usuarios/sesiones semilla (Ana Torres, Luis Pérez; ids en `/var/folders/wy/kf7vlr0s0013stp8jdglkst80000gn/T/opencode/sessions.json`, temporal).
...
- Pruebas: `src/**/*.test.ts` (1161; +57 con `TEST_DATABASE_URL`), `e2e/*.spec.ts` (139 funcionales + `visual.spec.ts` 24), líneas base en `e2e/__screenshots__/`.

## 8. Siguiente paso recomendado

Orden sugerido (del `PLAN_MEJORAS.md`):

1. **H2 en curso: #10 presentación hecho.** Seguir por `PLAN_MEJORAS.md` con H2: #12 notas/texto/regiones (L, primer paso de F2: figuras decorativas que no son servicios — nota adhesiva, texto libre, región de fondo — con su marca en el inspector, exportación y descripción accesible), #20 plantillas propias (S), #14 iconos en servidor (M), #9 comentarios anclados (L), #11 conectores editables (L).
