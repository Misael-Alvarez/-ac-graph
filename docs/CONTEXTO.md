...
- **Estado del árbol:** limpio en `be799e6` (portada según la referencia del usuario) sobre `506c8f0` (arreglos y rediseño). **Sin push** todavía: `origin/main` sigue en `3026f05`. Commits de esta etapa, por tema:
...
- Exportación draw.io: `src/lib/editor/drawio.ts` (`toDrawio`), `downloadDrawio` en `export.ts`, comando `exportDrawio` en `useCommands.ts`, fila en `ExportMenu.tsx`; iconos desde `spriteFor` a `data:image/svg+xml,<base64>`.
...
- Pruebas: `src/**/*.test.ts` (1498 con `TEST_DATABASE_URL`), `e2e/*.spec.ts` (169 funcionales + `visual.spec.ts` 24), líneas base en `e2e/__screenshots__/`; `audit:controls` 125, `audit:roles` 39.

## 8. Siguiente paso recomendado

Orden sugerido (del `PLAN_MEJORAS.md`):

1. **Hecho tras «que falta»:** draw.io (#17, mitad), diálogo «Nuevo diagrama» con plantillas propias, duplicados en el idioma. Queda **PPTX** (#17): sin dependencia nueva sería un ZIP propio (cabeceras locales + directorio central + CRC32) con `presentation.xml`, una diapositiva por vista y el PNG de cada vista (`rastersToPdf` ya rasteriza por vista); con `pptxgenjs` es más corto pero añade dependencia. Después #15 iconos Azure/OCI (descargar packs oficiales y revisar licencias) y #18/#19 (necesitan `ANTHROPIC_API_KEY`).
