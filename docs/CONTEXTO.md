...
- Pruebas: `src/**/*.test.ts` (1521 con `TEST_DATABASE_URL`; 1445 sin), `e2e/*.spec.ts` (170 funcionales + `visual.spec.ts` 24), líneas base en `e2e/__screenshots__/`; `audit:controls` 126, `audit:roles` 39.

## 8. Siguiente paso recomendado

Orden sugerido (del `PLAN_MEJORAS.md`):

1. **H2 casi cerrado:** #17 completo (draw.io y PPTX sin dependencia). Quedan #15 iconos oficiales Azure/OCI (descargar los packs oficiales, revisar licencias, mapear ~260 claves en `scripts/refreshIcons.mjs`) y #18/#19 IA que edita y revisión asistida (necesitan `ANTHROPIC_API_KEY` para verificarse). Después, H3 (CRDT, PWA, multi-workspace, escaneo cloud, diagramas generales) o el backlog menor de §3.
