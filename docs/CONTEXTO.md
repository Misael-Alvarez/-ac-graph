...
- **Estado del árbol:** cierre del 2026-09-15 empujado a `origin/main` (ver los tres últimos commits de la lista). Commits de esta etapa, por tema:
  ...
  - (cierre) — Lay the ground on AWS: Terraform for the whole stack, a deploy workflow, the guides; and sweep the repository
...
- **AWS (2026-09-15):** la base está en `deploy/aws/terraform` (ECS Fargate + ALB/ACM + RDS 17 + Secrets + ECR + rol OIDC para GitHub) y `.github/workflows/deploy-aws.yml`; **no se ha aplicado** desde esta máquina (sin credenciales, a propósito). Decisiones: sin NAT (tareas en subredes públicas con SG solo desde el ALB), `verify-full` hacia RDS con el bundle de Amazon **dentro de la imagen** (`deploy/aws/rds-global-bundle.pem`; `.dockerignore` lo readmite al final, después de `**/*.pem`), Terraform es dueño de la forma de la task definition y CI de la etiqueta (`ignore_changes = [task_definition]`), `idle_timeout` 3600 por los SSE. Guías: `docs/AWS.md` (inglés, runbook) y `docs/GUIA_RAPIDA_AWS.md` (español, para quien lo monta).
